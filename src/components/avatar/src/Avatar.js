...
const Avatar = ({
  ...
  const stackedAvatarItem = (avatars, size) => (
    <div style={styles.stackedContainer} {...props}>
      <div
        data-testid="stacked-avatar-container"
        className={`stacked-avatar-item ${className}`}
      >
        {avatars?.slice(0, visibleAvatars)?.map((avatar, index) => (
          <div
            key={index}
            className={`stacked-avatar-item-image ${stackedItemSize[size]} ${avatar.colorScheme} ${size}`}
          >
            {avatar.imageSrc ? (
              // eslint-disable-next-line jsx-a11y/alt-text
              <img src={avatar?.imageSrc} />
            ) : (
              <p data-testid="stack-letter-avatar-item">
    ...
    <>
      {avatars ? (
        ...
        <div className={`container ${className}`} {...props}>
          <div
            data-testid="letter-avatar-container"
            className={`avatar ${size} ${colorScheme}`}
          >
            {imageUnAvailable ? (
              <p data-testid="letter-avatar">{initial}</p>
            ) : (
              <img data-testid="avatar" src={src} alt="" />
            )}
            {status && (
              <div
